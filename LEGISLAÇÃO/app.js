...
const AppState = {
  ...
  getCurrentTopicData() {
    return this.currentTopic && window.DATA_LEGISLACAO?.[this.currentTopic];
  }
};
...
const Progress = {
  ...
  getStats() {
    if (!window.DATA_LEGISLACAO) return { total: 0, completed: 0, percentage: 0 };
    
    const topics = Object.keys(window.DATA_LEGISLACAO);
...
function selectTopic(topic) {
  if (!window.DATA_LEGISLACAO?.[topic]) {
...
function renderCurrentView() {
  ...
  if (!AppState.currentTopic || !window.DATA_LEGISLACAO?.[AppState.currentTopic]) {
    area.innerHTML = `
      <div class="empty">
  ...
  const data = window.DATA_LEGISLACAO[AppState.currentTopic];
...
function renderProgresso(area) {
  if (!window.DATA_LEGISLACAO) return;
  ...
  area.innerHTML = `
    <div class="progress-view fade-in">
      ...
      <div class="topics-grid">
        ${Object.entries(window.DATA_LEGISLACAO).map(([topic, data]) => {
...
function renderSidebar(filter = '') {
  ...
  if (!list || !window.DATA_LEGISLACAO) return;

  const filterLower = filter.toLowerCase();
  const fragment = document.createDocumentFragment();
  let found = false;

  for (const [topic, data] of Object.entries(window.DATA_LEGISLACAO)) {
...
function initializeApp() {
  ...
  if (window.DATA_LEGISLACAO) {
    renderSidebar();
    Progress.updateUI();
    renderCurrentView();
    updateFloatingButtons(); // Inicializar botões flutuantes
  } else {
    console.error('❌ window.DATA_LEGISLACAO não encontrado — verifique data_legislacao.js');
  }
}
...
function updateFloatingButtons() {
  ...
  if (topicCount && window.DATA_LEGISLACAO) {
    const totalTopics = Object.keys(window.DATA_LEGISLACAO).length;
    topicCount.textContent = totalTopics;
  }
  
  // Mostrar/ocultar botão de marcar progresso
  if (fabToggle) {
    const hasTopic = AppState.currentTopic && window.DATA_LEGISLACAO?.[AppState.currentTopic];
